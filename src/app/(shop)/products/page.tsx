...
export default async function ProductsPage({ searchParams }: ProductsPageProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-gray-200 dark:border-slate-800 mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-slate-100 tracking-tight">
            {searchParams.query ? (
              <span>
                Search Results for &ldquo;<span className="text-brand-700 dark:text-emerald-400">{searchParams.query}</span>&rdquo;
              </span>
        ...
        <div className="flex-1 w-full">
          {productsData.data.length === 0 ? (
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 p-12 text-center">
              <h3 className="text-lg font-bold text-gray-900 dark:text-slate-100">No products matched your search criteria</h3>
              ...
              <Link
                href="/products"
                className="inline-block mt-4 text-xs font-semibold text-brand-700 dark:text-emerald-300 bg-brand-50 dark:bg-emerald-950/60 px-4 py-2 rounded-lg hover:bg-brand-100 dark:hover:bg-emerald-900/60 transition"
              >
                Clear All Filters
              </Link>
            </div>
                ...
                return (
                  <Link
                    ...
                    className={`w-9 h-9 rounded-xl flex items-center justify-center text-xs font-bold transition ${
                      isCurrent
                        ? 'bg-brand-700 dark:bg-emerald-600 text-white shadow'
